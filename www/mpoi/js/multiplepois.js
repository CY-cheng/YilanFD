...
var World = {
	...
	requestDataFromLocal: function requestDataFromLocalFn(centerPointLatitude, centerPointLongitude) {
		var poisToCreate = contentData.length;
		var poiData = [];
		...
		for (var i = 0; i < poisToCreate; i++) {
			long = contentData[i].long;
			lat = contentData[i].lat;
			// long = centerPointLongitude + (Math.random() / 5 - 0.1);
			// lat = centerPointLatitude + (Math.random() / 5 - 0.1);	
	
			if(contentData[i].title.length > 5) {
				title = contentData[i].title.substr(0,5);
			} else {
